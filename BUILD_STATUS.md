...
- The checked-in `korail_engine.py` is integrated with Chaquopy for route-based KORAIL monitoring, reservation, and explicit opt-in one-shot card payment through the current mobile endpoint mapping; failed attempts fall back to official app/web reservation-list payment and bounded paid-ticket verification until the reservation deadline, including recovery of an existing pending reservation after service restart.
