...
- `android/app/src/main/python/korail_engine.py`
  ...
  - 공식 결제 후 `tickets()`로 같은 구간·열차번호의 발권 목록을 재조회하는 결제 확인 경로 제공
...
- KORAIL 공식 보도자료는 반복 자동입력 매크로를 차단·제재한다고 안내하므로, 조회 간격은 30~60초로 제한하고 좌석 발견·예약 후 즉시 종료한다. 결제 화면을 비공식 UI 자동화로 조작하지 않는다.
...
- 결제 확인 시 예약 알림의 열차번호까지 일치시키는 오탐 방지 반영
